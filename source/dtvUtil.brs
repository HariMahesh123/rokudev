REM ************************************************************
REM Some Utility functions. Obtained from youtube example
REM ******************************************************
REM 
...
Function CreateURLTransferObject(url As String, gDataHeader As String) as Object
    obj = CreateObject("roUrlTransfer")
    obj.SetPort(CreateObject("roMessagePort"))
    obj.SetUrl(url)
    obj.AddHeader("X-GData-Device", gDataHeader)
...
Function NewHttp(url As String, gDataHeader As String) as Object
    obj = CreateObject("roAssociativeArray")
    obj.Http                        = CreateURLTransferObject(url, gDataHeader)
    obj.FirstParam                  = true
...
Function NewHttp2(url As String, contentHeader As String) as Object
    obj = CreateObject("roAssociativeArray")
    obj.Http                        = CreateURLTransferObject(url, contentHeader)
    obj.FirstParam                  = true
    obj.AddParam                    = http_add_param
    obj.AddRawQuery                 = http_add_raw_query
    obj.GetToStringWithRetry        = http_get_to_string_with_retry
...
Function http_get_to_string_with_retry(gDataHeader As String) as String
    timeout%         = 1500
    num_retries%     = 5
...
'        print "httpget try " + itostr(num_retries%)
        if (m.Http.AsyncGetToString())
            ...
            elseif event = invalid
                ...
                m.Http = CreateURLTransferObject(m.Http.GetUrl(), gDataHeader)
                timeout% = 2 * timeout%
            else
